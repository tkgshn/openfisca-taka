...
install: deps
	...
	@# `make install` installs the editable version of openfisca-taka.
...
build: clean deps
	...
	python -m build
	pip uninstall --yes openfisca-taka
...
test: clean
	openfisca test --country-package openfisca_taka openfisca_taka/tests

serve-local: build
	openfisca serve --country-package openfisca_taka
